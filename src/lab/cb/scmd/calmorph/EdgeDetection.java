...
public class EdgeDetection {
	...
	public void edge(int startid) {
		...
		Labeling lab = new Labeling(_width, _size, 200, true);  //ÇQÇOÇOà»è„ÇÃâÚÇç◊ñEÇ∆Ç›Ç»Ç∑
		...
		for(int i=0;i<labeled.length;i++) {
			...
			for(int j=0;j<labeled[i].size();j++) {
				...
				pixeltocell[p]=i;
				if(_points[p-_width] == 0 && _points[p-1] == 0 && _points[p+1] == 0 && _points[p+_width] == 0) { //vecÇ…ÇÕç◊ñEÇÃïîï™ÇÃÇ§ÇøÇ”ÇøÇæÇØÇãLâØ
					labeled[i].remove(j);
					j--;
				}
			}
		...
		for(int i=0;i<labeled.length;i++) { //imageÇç◊ñEÇÃÇ”Çøïîï™ÇæÇØçïÇ≈écÇËÇ™îíÇ∆Ç»ÇÈÇÊÇ§Ç…ÉZÉbÉg
			for(int j=0;j<labeled[i].size();j++) {
				_points[((Integer)labeled[i].get(j)).intValue()] = 0;
			}
		}
		for(int i=0;i<labeled.length;i++) {//ç◊ñEì‡ïîÅió÷äsÇÊÇËì‡ë§ÅjÇ…ê⁄ÇµÇƒÇ¢Ç»Ç¢Ç‡ÇÃÇÕÇ∑ÇƒÇÈ
			for(int j=0;j<labeled[i].size();j++) {
				int p=((Integer)labeled[i].get(j)).intValue();
		...
		for(int i=0;i<cell.length;i++) {//àÍì_ÇæÇØã…í[Ç…ñæÇÈÇ¢ç◊ñEÇcomplexÇ… ‡Và‰í«â¡ïîï™
	...
	private static void edgecorrect2(Vector[] vec,int[] image,int[] oriimage, int[] pixeltocell2, Cell[] cell, int _width) {
		...
		double counter2 = 0;//Ç⁄Ç‚ÇØâÊëúèúãéÇÃÇΩÇﬂÇÃÉJÉEÉìÉ^Å@‡Và‰í«â¡ïîï™
	    for(int i=0;i<vec.length;i++) {
	    	int j=0;
	        while(j<vec[i].size()) {
	        	n = vec[i].size();
	        	...
	        	while(j<n){
	            	...
	            	if(brightness1 >= brightness2 || oriimage[p-x*(xx-1)] - oriimage[p-x*xx] < 5){ // í«â¡
	                move[p] = true;
	            	}
	        		j++;
	        	}
			...
			double counter = 0;//Ç±ÇÃêÊÅAÇ⁄Ç‚ÇØâÊëúèúãéÇÃëÄçÏÅ@‡Và‰í«â¡ïîï™
			for(k=0;k<vec[i].size();k++){
				int p=((Integer)vec[i].get(k)).intValue();
				x=0;
				if(pixeltocell2[p-1] != i && image[p-1] == 255) x -= 1;
