...
public class BudValidation {
	...
	public static Cell[] validation(Cell[] cells, final int size, final int width) {
		for ( int i = 0; i < cells.length; i++ ) {
			...
			if ( budPerpendicularLengthValidationPrimary(neck_length, bud_perpendicular_length) ) { deleteBudEdge(cells, i); continue; }
			/*
			...
			*/
			if ( motherLengthValidation(cells[i].mother_edge.size(), cells[i].bud_edge.size()) && 
					( budPerpendicularLengthValidation(neck_length, bud_perpendicular_length) ||
							budLengthValidation(neck_length, cells[i].bud_edge.size()) ) ) { deleteBudEdge(cells, i); } 
		}
		return cells;
	}
	...
	protected static boolean budLengthValidation(double neck_length, double bud_edge_length) {
		if ( neck_length * _bud_length_threshold > bud_edge_length ) { return true; }
		return false;
	}
	...
	protected static boolean budPerpendicularLengthValidation(double neck_length, double bud_perpendicular_length) {
		if ( neck_length * _perpendicular_length_threshold > bud_perpendicular_length ) { return true; }
		return false;
	}
