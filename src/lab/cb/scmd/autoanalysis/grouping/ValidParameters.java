...
public class ValidParameters  implements DataFileName {
	...
	final static int	OPT_WILDDIR		    = 1;
	final static int	OPT_MUTANTDIR		= 2;
	/**
	 * @param args
	...
	public void setupByArguments(String[] args) throws SCMDException
	{
		...
		if(_parser.isSet(OPT_MUTANTDIR))
			mutant_dir = new String(_parser.getValue(OPT_MUTANTDIR));
		if(_parser.isSet(OPT_WILDDIR))
			wildtype_dir=new String(_parser.getValue(OPT_WILDDIR));
	...
	String _dir_name;
	java.util.HashMap	_parameterToDataTable;
	java.util.Vector    _yukoParameterList;
	java.util.Vector    _yukoDataTable;
...
//	4.出力する。
	private void init(){
		_parameterToDataTable	= new java.util.HashMap();
		_yukoParameterList      = new java.util.Vector();
		_yukoDataTable          = new java.util.Vector();		
	}
	...
	private void readFile(String filename)throws SCMDException{
		...
		while(i.hasNext()){
			...
			TableIterator ti=(TableIterator)cmt.getVerticalIterator(parameter_name);
			java.util.Vector verticalData=new java.util.Vector(rowSize);
			while(ti.hasNext()){
	...
	private void makeYukoDataTable(){
		java.util.Iterator i=_yukoParameterList.iterator();
		while(i.hasNext()){
			String parameter=(String)(i.next());
			java.util.Vector verticalData=(java.util.Vector)_parameterToDataTable.get(parameter);
	...
	private void printYukoDataTable() throws IOException{
		...
		for(int i=0;i<_yukoParameterList.size();++i){
			outfile.print((String)_yukoParameterList.get(i)+t);
		...
		int tmp=0;
		for(int row=0;row<((java.util.Vector)_yukoDataTable.get(0)).size();++row){
			...
			for(int i=0;i<_yukoDataTable.size();++i){
				...
				java.util.Vector verticalData=(java.util.Vector)(_yukoDataTable.get(i));
				Cell c=(Cell)(verticalData.get(row));
...
//以下は有効パラメーター決定プログラム
	...
	private java.util.Vector transpose_ToDouble_IgnoreNaN(java.util.Vector v){
		java.util.HashSet missingValueSet = new java.util.HashSet();
	...
	private java.util.Vector getHistogram(java.util.Vector data, double min,double max,int n_box)
	{
		double intervalSize=(max-min)/(double)n_box;
		java.util.Vector histogram=new java.util.Vector();
		...
		for(int i=0;i<data.size();++i){
			...
			int count=((Integer)(histogram.get(boxNum))).intValue();
	...
	private double getMode(java.util.Vector data){
		...
		java.util.Vector histogram=getHistogram(data,min,max,number_of_partitions);
		int max_data=0;
		double mode=0;
		for(int i=1;i<number_of_partitions;++i){
			int num_of_data=((Integer)histogram.get(i)).intValue();
