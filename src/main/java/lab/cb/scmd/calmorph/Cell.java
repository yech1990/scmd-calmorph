...
//			Dpointparam[i] = new Point(-1, -1);
    ...
    void setAState(int diff, int[] AImage) {
        ...
        if (!Agroup.equals("N") && group > 0) {
            ...
            int[] copy = actinpatchorder.clone();
            for (int i = 0; i < actinpatchpoint.size(); i++) {
    ...
    boolean inmother(int p) {
        if (group > 1) {
            ...
            return (dx * (p / w) - dy * (p % w) + dy * (neck[0] % w) - dx
                    * (neck[0] / w))
    ...
    private boolean inmother(Point p) {
        if (group > 1) {
            int dx = neck[1] % w - neck[0] % w;
            int dy = neck[1] / w - neck[0] / w;
            return (dx * (p.y) - dy * (p.x) + dy * (neck[0] % w) - dx
                    * (neck[0] / w))
                    * (dx * budtop.y - dy * budtop.x + dy * (neck[0] % w) - dx
                    * (neck[0] / w)) <= 0;
        } else {
    ...
    private Point Intersection2(Point p1, Point p2, Point p3, Point p4) {
        if ((p4.y - p3.y) * (p2.x - p1.x) - (p2.y - p1.y) * (p4.x - p3.x) != 0) {
            ...
            double y;
            if (p2.x != p1.x)
                y = (double) p1.y + ((p2.y - p1.y) * (x - p1.x))
                        / (double) (p2.x - p1.x);
            else
                y = (double) p3.y + ((p4.y - p3.y) * (x - p3.x))
                        / (double) (p4.x - p3.x);
